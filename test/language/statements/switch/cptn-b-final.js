...
assert.sameValue(
  eval('1; switch ("a") { default: case "a": 2; }'),
  2,
  'non-empy StatementList (lone case)'
);
assert.sameValue(
  eval('1; switch ("b") { default: case "a": case "b": }'),
  undefined,
  'empty StatementList (following an empty case)'
);
assert.sameValue(
  eval('1; switch ("b") { default: case "a": case "b": 2; }'),
  2,
  'non-empty StatementList (following an empty case)'
);
assert.sameValue(
  eval('1; switch ("b") { default: case "a": 2; case "b": }'),
  undefined,
  'empty StatementList (following a non-empty case)'
);
assert.sameValue(
  eval('1; switch ("b") { default: case "a": 2; case "b": 3; }'),
  3,
